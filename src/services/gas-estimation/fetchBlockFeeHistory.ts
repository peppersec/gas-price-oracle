...
import { fromNumberToHex } from '@/utils'
/**
...
export default async function fetchBlockFeeHistory<Percentile extends number>({
  ...
  let finalEndBlockNumber: BigNumber | undefined
  if (givenEndBlock === 'latest') {
    const { data: latestBlock } = await fetcher.makeRpcCall<{ result: string }>({
      method: 'eth_blockNumber',
      params: [],
    })
    const blockBn = new BigNumber(latestBlock.result)
    finalEndBlockNumber = blockBn.isNaN() ? blockBn : new BigNumber(0)
  } else {
  ...
  return blockChunks.reduce((array, blocks) => [...array, ...blocks], [] as FeeHistoryBlock<Percentile>[])
}
...
function buildExistingFeeHistoryBlock<Percentile extends number>({
  ...
  const priorityFeesByPercentile = percentiles.reduce((obj, percentile, percentileIndex) => {
    ...
    return { ...obj, [percentile]: new BigNumber(priorityFee) }
  }, {} as Record<Percentile, BigNumber>)
 ...
 */
function buildNextFeeHistoryBlock({ baseFeePerGas, number }: { baseFeePerGas: BigNumber; number: BigNumber }) {
  return {
    number,
    baseFeePerGas,
    gasUsedRatio: null,
...
async function makeRequestForChunk<Percentile extends number>({
  ...
  const numberOfBlocksHex = fromNumberToHex(numberOfBlocks)
  const endBlockNumberHex = fromNumberToHex(endBlockNumber)

  const { data } = await fetcher.makeRpcCall<{ result: EthFeeHistoryResponse }>({
    method: 'eth_feeHistory',
    params: [numberOfBlocksHex, endBlockNumberHex, percentiles],
  })

  const response = data.result

  const startBlockNumber = new BigNumber(response.oldestBlock)
  ...
  if (
    ...
    return baseFeesPerGasAsHex.map((baseFeePerGasAsHex, blockIndex) => {
      const baseFeePerGas = new BigNumber(baseFeePerGasAsHex)
      const number = startBlockNumber.plus(blockIndex)
 ...
 */
function determineRequestChunkSpecifiers(endBlockNumber: BigNumber, totalNumberOfBlocks: number): RequestChunkSpecifier[] {
  if (endBlockNumber.isLessThan(new BigNumber(totalNumberOfBlocks))) {
    totalNumberOfBlocks = endBlockNumber.toNumber()
  }

  const specifiers = []
  for (
    let chunkStartBlockNumber = endBlockNumber.minus(totalNumberOfBlocks);
    chunkStartBlockNumber.lt(endBlockNumber);
    chunkStartBlockNumber = chunkStartBlockNumber.plus(MAX_NUMBER_OF_BLOCKS_PER_ETH_FEE_HISTORY_CALL)
  ) {
    const distanceToEnd = endBlockNumber.minus(chunkStartBlockNumber).toNumber()
    const numberOfBlocks =
      distanceToEnd < MAX_NUMBER_OF_BLOCKS_PER_ETH_FEE_HISTORY_CALL
        ? distanceToEnd
        : MAX_NUMBER_OF_BLOCKS_PER_ETH_FEE_HISTORY_CALL
    const chunkEndBlockNumber = chunkStartBlockNumber.plus(numberOfBlocks)
